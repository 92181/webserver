...
unsigned char *b,*c,qw[]="\033[31mMemory Allocation Failure!\033[0m\n",qe[]="\033[31mFile Operation Failed!\033[0m\n",*u,*t,*j,**gh,**sf,**se;

char vq[16384],*vz,*vw,*vy,*gi="index.html",*gu;
...
{
  // Allocate Buffer Memory (May Overflow);
  char st[131072],*y=st,*pt[2048],**jx=pt,*cd=malloc(1024),**ui=pt+sizeof(pt),*op=st+sizeof(st),*yp=cd+sizeof(cd);
  
  t=u=malloc(1024*1024);j=u+1024*1024;
  ...
  *jx=y;
  ...
  while(jx>=pt)
  {
    // Write Current Directory To Active Buffer;
    g=cd;y=*jx;
    ...
    if(py>yp)
    {
      unsigned int nws=(yp-cd)+2048;cd=g=realloc(cd,nws);
    ...
    *g=0;y=*jx;jx-=1;
    ...
    while((en=readdir(dr))!=0)
    {
      ...
      char *v=en->d_name,*tu=g,*ty=tu+srl(v),*lr;
